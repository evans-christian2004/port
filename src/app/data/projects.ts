...
<ul>
  ...
  <li>Developed an <strong>AI workflow</strong> where a root coordinator routes tasks between two agents: 
    <ol>
      <li>A Q&A agent that explains financial insights.</li>
      <li>A calendar agent that modifies events in real time.</li>
    </ol>
  </li>
</ul>
...
<p>
  I learned how to coordinate multiple AI agents within a product workflow, manage session state over time, and connect emerging AI infrastructure with user-friendly interfaces.
</p>
    `,
...
<h2>Contributions</h2>
<ul>
  <li>Set up the modular pipeline in <strong>Next.js API Routes</strong> and <strong>TypeScript</strong> to handle ingest → extract → summarize → export.</li>
  <li>Implemented mock and real <strong>AI summarization</strong> layers so the app works locally without API keys and scales up when real APIs are connected.</li>
  <li>Developed the <strong>React UI</strong> with <strong>Tailwind CSS</strong> including tabs for Upload/Paste modes, progress states, and error feedback.</li>
...
<p>
  A better approach (and what our team originally planned) would have been to use the Google ADK to initialize agents that process the tasks for image and summary generation in parallel, which would have sped up loading times significantly. Unfortunately, we ran into a time-crunch trying to impliment a new technology that we had never seen before in 3 days, so we decided to impliment the pipeline using the standard Google Gemini API and fetched responses sequentaially.
</p>

<h2>What I Learned</h2>
<p>
  I gained experience in many areas from my first large-scale hackathon. Project time and scope management, Next.js and TypeScript concept reinforcement, and my first real dive into applied AI development.
</p>
    `,
...
<h2>Challenges and Wins</h2>
<p>
  We started off trying Excalidraw but ran into styling and integration issues with Vite, so switching to TLDraw made things smoother. The hardest part was reliably sending and grading whiteboard images, but we made it work and got real grading feedback in the app. The application wasn't fully realized by the end of the 24 hour hackathon, but I'm proud that we were able to make a functional MVP by the end with a team of students who are new to web development.
</p>
...
<ul>
  <li>Set up a <strong>Sanity Studio</strong> with custom schemas for pages, projects, and reusable blocks of content.</li>
  ...
  <li>Explored <strong>CMS workflows</strong> like <strong>content versioning</strong>, <strong>custom roles</strong>, and <strong>team collaboration</strong> features.</li>
</ul>
...
<h2>Challenges and Wins</h2>
<p>
  The challenge was moving away from the typical CMS mindset (like WordPress) into content-as-data and learning how to flow that into a frontend. I'm proud I ended up with a working demo where content could be added without editing code.
</p>

<h2>What I Learned</h2>
<p>
  I learned how headless CMS platforms differ from traditional systems by offering structured APIs instead of templates. I found how to decouple content from presentation, making it easier to scale and maintain websites. This project gave me confidence that modern CMS workflows are both flexible and dev-friendly. Definitely keeping this in the toolbelt if I need to develop an application for someone with easy CMS management.
</p>
    `,
...
<ul>
  ...
  <li>Developed the UI with <strong/>Vite, JavaScript, HTML, and CSS</strong> ensuring animations are smooth and it works across devices.</li>
...
<h2>Challenges and Wins</h2>
<p>
  The main challenge was aligning the flip-card animation with the state changes of question/answer ensuring flips don't glitch and logic stays in sync. I ended up with a study tool that's fun and helps with memory recall instead of just passive reading. Now I have to actually make a project with TypeScript.
</p>

<h2>What I Learned</h2>
<p>
  This project improved my skills in interactive UI design, state management with React, and building focused learning tools. It showed me that even small, well-designed apps can deliver real value when the user experience is tight and workflow-driven. This projet was a good way to build off the skills I learned by adding in interactive elements using JavaScript.
</p>
    `,
  ...
  },
  {
    ...
    bodyText: `
<p>
  I built <strong>305 Life</strong>, a community board showcasing local Miami events and spots. The idea was to practice React fundamentals while making something personal and useful. This was originally an assignment for my <strong>CodePath certification</strong>, but I decided to make it a little more personal.
</p>

<h2>Accomplishments</h2>
<ul>
  <li>Developed a <strong>responsive</strong> grid layout that displays over ten unique Miami events using reusable React components.</li>
...
<h2>Challenges and Wins</h2>
<p>
  A big challenge was managing the ace logic (when to count an 11 as 1) and making sure the dealer drew correctly and stopped at the right point. I'm proud the final version handles all major paths (player win, dealer win, tie) and gives a smooth browser-based game experience without external libraries. This is also the first project I've made that isn't just for an assignment or following a tutorial.
</p>

<h2>What I Learned</h2>
<p>
  This project reinforced my fundamentals in JavaScript state management, DOM manipulation, and UI control logic. It also reminded me that even small, focused projects have big value. It strengthened my understanding of flow control, randomness, user interaction, and game logic, all of which apply to larger apps.
</p>
    `,
